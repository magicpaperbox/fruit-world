...
class Modal:
    ...
    def draw(self, surf: pygame.Surface):
        ...
        if not self.transparent_background:
            pygame.draw.rect(surf, self.theme.modal_bg, self.rect, border_radius=self._border_radius)
            if self.show_border:
                shadow_rect = self.rect.inflate(3, 3).move(2, 2)
                pygame.draw.rect(
                    surf, self.theme.modal_shadow, shadow_rect, width=GameUnit(6).non_zero_pixels, border_radius=self._border_radius
                )
                pygame.draw.rect(
                    surf, self.theme.modal_border, self.rect, width=GameUnit(4).non_zero_pixels, border_radius=self._border_radius
                )

        if self.title:
            title = self.font.render(self.title, True, self.theme.text_color)
